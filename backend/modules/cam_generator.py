...
def _request_gemini_narrative(prompt: str) -> Optional[str]:
...
def generate_cam_content(analysis_data: Dict[str, Any]) -> Dict[str, str]:
    ...
    Each section is generated by:
      ...
      3. Calling the Gemini API for a fluent narrative.
      4. Falling back to the original formatted-string template if the API
         is unavailable.

    The **final_recommendation** section additionally includes the top SHAP
    risk factors so the narrative explicitly justifies the recommended limit
    and pricing based on model explainability.
    """
    ...
    narrative = _request_gemini_narrative(_build_cam_section_prompt("Executive Summary", exec_data))
    sections["executive_summary"] = narrative or _fallback_executive_summary(
        company, industry, decision, features, summary, risk, premium, loan_amount, recommended_limit,
    )
    ...
    narrative = _request_gemini_narrative(_build_cam_section_prompt("Character", char_data))
    sections["character"] = narrative or _fallback_character(features, web)
    ...
    narrative = _request_gemini_narrative(_build_cam_section_prompt("Capacity", cap_data))
    sections["capacity"] = narrative or _fallback_capacity(features)
    ...
    narrative = _request_gemini_narrative(_build_cam_section_prompt("Capital", capital_data))
    sections["capital"] = narrative or _fallback_capital(features, capital_impact)
    ...
    narrative = _request_gemini_narrative(_build_cam_section_prompt("Collateral", coll_data))
    sections["collateral"] = narrative or _fallback_collateral(features, stress)
    ...
    narrative = _request_gemini_narrative(_build_cam_section_prompt("Conditions & Covenants", cond_data))
    sections["conditions"] = narrative or _fallback_conditions(web, stress, features, covenants)

    # ── Final Recommendation (with SHAP justification) ─────────────────
    rec_data = {
        "Decision": decision.get("decision", "PENDING"),
        "Recommended Limit": _fmt_inr(recommended_limit),
    ...
    narrative = _request_gemini_narrative(
        _build_cam_section_prompt("Final Recommendation", rec_data, shap_factors=risk_factors),
    )
