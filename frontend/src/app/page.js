...
import {
  ...
  BrainCircuit, Download, Activity, Scale, Network, ShieldCheck, TrendingUp, AlertTriangle, FileBarChart, ArrowRight
} from "lucide-react";
import {
...
export default function Workspace() {
  ...
  const isAnalyzing_ = useState(false);
  const [isAnalyzing, setIsAnalyzing] = isAnalyzing_;

  // Global Store - replaces local analysis/notes state
  const storeAnalysisResult = useAppStore(s => s.activeAnalysisResult);
  const storeResearchNotes = useAppStore(s => s.activeResearchNotes);
  const storeSetAnalysisResult = useAppStore(s => s.setAnalysisResult);
  const storeSetResearchNotes = useAppStore(s => s.setResearchNotes);

  // Aliases for backwards compat with existing JSX
  const analysisResult = storeAnalysisResult;
  const researchNotes = storeResearchNotes;
  const setResearchNotes = storeSetResearchNotes;
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const data = await res.json();
      storeSetAnalysisResult(data, payload.analysis_id);
    } catch (err) {
      console.error(err);
      // Fallback: save to global store even when backend is down
      storeSetAnalysisResult({
  ...
  const handleViewCAM = () => {
    const id = storeAnalysisId || "draft";
    router.push(`/cam-terminal/${id}`);
  };
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
        ...
        >
          <Card className="flex-1 shadow-lg border-border/40 flex flex-col relative overflow-hidden">
            ...
            <CardFooter className="pt-0 pb-6 px-6 relative z-20 bg-card flex flex-col gap-2">
              <Button
              ...
              {analysisResult && (
                <Button
                  ...
                  className="w-full shadow-sm bg-indigo-600 hover:bg-indigo-500 text-white"
                >
